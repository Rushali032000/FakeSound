...
def parse_args():
    ...
    parser.add_argument(
        "--test_files", '-f', default={
            "easy": f"{WORKSPACE_PATH}/deepfake_data/output2.json",
        }
    )
    parser.add_argument(
...
def main():
    ...
    model.load_state_dict(torch.load(args.model_pt, map_location=torch.device('cpu')))  # Load model to CPU
    ...
    for test_set, test_file in args.test_files.items():
        ...
        for step, batch in tqdm(enumerate(dataloader)):
            ...
            with torch.no_grad():
                output = model(audio.to(device))  # Ensure audio is on CPU
            ...
            for idx, _ in enumerate(pred):         
                ...
                if binary_label[idx] == 0:
                    ...
                    try:
                        # Attempt to split audio_id[idx][12:] by '_'
                        parts = audio_id[idx][12:].split("_")
                        if len(parts) != 2:
                            print(f"Warning: Invalid audio_id format '{audio_id[idx]}'. Expected 'something_onset_offset', got '{audio_id[idx][12:]}'. Setting ref_onset, ref_offset to 0.0.")
                            ref_onset, ref_offset = 0.0, 0.0
                        else:
                            ref_onset, ref_offset = float(parts[0]), float(parts[1])
                    except Exception as e:
                        print(f"Error processing audio_id '{audio_id[idx]}': {e}. Setting ref_onset, ref_offset to 0.0.")
                        ref_onset, ref_offset = 0.0, 0.0
                ref_list.append({
                    'event_label': 'fake',
                    'onset': ref_onset,
